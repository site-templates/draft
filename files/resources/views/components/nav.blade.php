...
<header id="header" class="fixed inset-x-0 top-0 z-50">
    <div class="mx-auto flex h-16 w-full max-w-6xl items-center gap-6 px-6">
        ...
        <a href="/" aria-label="Homepage" class="flex shrink-0 items-center gap-2.5 text-ink">
            <svg viewBox="0 0 173 91" fill="none" class="h-4 w-auto shrink-0" aria-hidden="true">
        ...
        <nav class="ml-auto max-lg:hidden" aria-label="Main">
            <ul role="list" class="flex items-center gap-1 text-sm text-muted">
                ...
                <li class="relative" data-dropdown>
                    ...
                    {{--
                        Centered under the trigger with left-1/2 plus a negative
                        margin of half the panel width — the open/close motion
                        animates the translate property, so translate utilities
                        can't be used for positioning here.
                    --}}
                    <div data-dropdown-panel class="absolute left-1/2 top-full -ml-[18rem] w-[36rem] pt-3">
                        <div class="grid grid-cols-[1.15fr_1fr] gap-1 rounded-2xl border border-line bg-panel/95 p-1.5 shadow-2xl shadow-black/50 backdrop-blur-xl">
                            ...
                            <div class="flex flex-col rounded-xl bg-raised/60 p-4">
                                <p class="font-mono text-[11px] font-medium tracking-widest text-faint uppercase">Latest</p>

                                <a href="/updates/introducing-draft-agents" class="group mt-3 block">
                                    <span class="block overflow-hidden rounded-lg border border-line">
                                        <img src="https://assets.ui.sh/wallpapers/silk.webp?variant=midnight-violet" alt="" class="aspect-video w-full object-cover grayscale transition-transform duration-500 group-hover:scale-[1.04]" loading="lazy">
                                    </span>
                                    <span class="mt-2.5 block text-[13px] font-medium leading-snug text-ink transition-colors duration-200 group-hover:text-accent">Introducing Draft Agents</span>
                                    <span class="mt-0.5 block text-xs text-faint">Feb 12, 2026</span>
                                </a>

                                <a href="/updates" class="group mt-auto inline-flex items-center gap-1 pt-4 text-[13px] font-medium text-muted transition-colors duration-200 hover:text-ink">
                                    All updates
        ...
        <!-- The separator between the links and the auth actions -->
        <span class="h-4 w-px bg-muted/25 max-lg:hidden" aria-hidden="true"></span>

        <!-- Sign in + CTA -->
        <div class="flex items-center gap-2.5 max-lg:hidden">
            <a href="{{ $signInLink }}" class="rounded-full px-3 py-2 text-sm text-muted transition-colors duration-200 hover:text-ink">{{ $signInText }}</a>
            <a href="{{ $ctaLink }}" class="rounded-full bg-ink px-4 py-2 text-sm font-medium text-canvas shadow-[0_0_0_1px_rgba(255,255,255,0.06)] transition-colors duration-200 hover:bg-white">{{ $ctaText }}</a>
        </div>
        ...
        <button
            ...
            class="ml-auto flex size-10 cursor-pointer items-center justify-center rounded-lg text-ink transition-colors duration-200 hover:bg-raised lg:hidden">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.5" class="size-5 stroke-current [.menu-open_&]:hidden" aria-hidden="true">
                <path stroke-linecap="round" d="M3.75 7h16.5M3.75 12h16.5M3.75 17h16.5"/>
            </svg>
            <svg viewBox="0 0 24 24" fill="none" stroke-width="1.5" class="hidden size-5 stroke-current [.menu-open_&]:block" aria-hidden="true">
    ...
    <div data-mobile-panel class="border-b border-line bg-canvas/90 backdrop-blur-xl lg:hidden">
        <nav class="mx-auto w-full max-w-6xl px-6 py-4" aria-label="Mobile">
            ...
            <div class="mt-4 flex items-center gap-3 border-t border-line pt-4">
                ...
                <a href="{{ $ctaLink }}" class="flex-1 rounded-full bg-ink px-4 py-2.5 text-center text-sm font-medium text-canvas hover:bg-white">{{ $ctaText }}</a>
            </div>
        </nav>
    </div>
</header>
